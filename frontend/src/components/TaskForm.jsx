...
const TaskForm = ({ editingTask, setEditingTask, setTasks }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingTask) {
      ...
      const { data } = await axiosInstance.post('/api/complaints', form);
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto p-4 border rounded">
      ...
      <input name="email" value={form.email} onChange={handleChange} placeholder="Email" required className="w-full p-2 border mb-2" />
      ...
      <textarea name="description" value={form.description} onChange={handleChange} placeholder="Description" className="w-full p-2 border mb-2"></textarea>
      ...
      {/* NEW: Status dropdown in form */}
      <select name="status" value={form.status} onChange={handleChange} className="w-full p-2 border mb-2">
